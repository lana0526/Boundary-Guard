...
const AnalysisDashboard: React.FC<AnalysisDashboardProps> = ({ result, perspective }) => {
          ...
          <div className="flex-1 space-y-4">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-2xl font-bold text-slate-800">
                {hasManipulation ? 'Communication Risk Detected' : 'Healthy Communication Pattern'}
              </h2>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
            ...
            <div className="space-y-4">
              <div>
                <h4 className="text-sm font-semibold text-slate-500 uppercase tracking-wide mb-1">Psychological Impact</h4>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
              ...
              <div className="space-y-2">
                {result.highlighted_text.map((evidence, idx) => (
                  <EvidenceHighlight key={idx} evidence={evidence} />
                ))}
              </div>
            </div>
          )}
        ...
        <div className="bg-indigo-900/5 rounded-xl border border-indigo-100 p-6 h-fit">
          <h3 className="text-lg font-bold text-indigo-900 mb-4">
            {perspective === 'received' ? 'Suggested Responses' : 'Recommended Rewrites'}
          </h3>
          <p className="text-sm text-indigo-700 mb-6">
            {perspective === 'received' 
              ? "Here are some ways you can respond to maintain boundaries without escalating conflict."
              : "Consider rephrasing your message to express your needs clearly without manipulation."}
          </p>
          
          <div className="space-y-4">
            {suggestions?.map((s, i) => (
              <GuidanceCard key={i} suggestion={s} index={i} />
            ))}
            {(!suggestions || suggestions.length === 0) && (
              <div className="text-center py-8 text-slate-500">
                No specific suggestions needed for this text.
              </div>
            )}
          </div>
        </div>
